...
function ollamaError(error: unknown) {
  if (error instanceof Error && error.name === "AbortError") return "Ollama took too long to respond";
...
async function ollamaFetch(path: string, init?: RequestInit) {
  ...
  try {
    return await fetch(`${ollamaBaseUrl}${path}`, { ...init, signal: controller.signal });
  } finally {
    clearTimeout(timer);
  }
}
...
export async function GET() {
  try {
    ...
    const installed = data.models?.some((model) => model.name === ollamaModel) ?? false;
    if (!installed) throw new Error(`${ollamaModel} is not installed in Ollama`);
    return NextResponse.json({ online: true, model: ollamaModel }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
    return NextResponse.json({ online: false, model: ollamaModel, error: ollamaError(error) }, { status: 503 });
  }
}

export async function POST(request: Request) {
  try {
    const body = await request.json() as { message?: unknown; history?: unknown; snapshot?: HouseSnapshot };
    ...
    const history = Array.isArray(body.history)
      ...
      }).slice(-8).map((item) => ({ role: item.role, content: item.text.slice(0, 2000) }))
      : [];
    const context = body.snapshot ? JSON.stringify(body.snapshot) : "House telemetry was not provided.";
    ...
    return NextResponse.json({ answer, model: ollamaModel }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
    return NextResponse.json({ error: ollamaError(error) }, { status: 503, headers: { "Cache-Control": "no-store" } });
  }
}
